...
| RN-EST-11 | DEF | Un tutor tiene contacto válido cuando cuenta con nombre, relación y al menos uno de los siguientes datos con formato válido: correo electrónico o número telefónico. SIGE lo marca como válido automáticamente al validar el formato del dato disponible; el Personal administrativo o Administrador puede marcarlo como inválido con motivo. | Alta o edición de un tutor | Se asigna el indicador de contacto válido/inválido. | El correo sigue siendo el **único** canal para comunicaciones formales de reportes (RN-COM-01); un tutor con solo teléfono es "contacto válido" a efectos de RN-EST-03, pero no puede recibir comunicaciones automatizadas por RN-COM-01, solo ser contactado telefónicamente en caso de emergencia por el personal de la institución. El teléfono capturado no está garantizado como línea exclusiva de ese tutor (puede ser un número familiar compartido). | RF-EST-03, RN-EST-03, RN-COM-01 |
| RN-EST-12 | RES | El expediente del estudiante PUEDE registrar, de forma opcional: tipo de sangre, alergias, indicador de adeudo de pago y materias pendientes de ciclos anteriores ("debe materias"). El acceso a estos campos se restringe por rol: los datos de salud (tipo de sangre, alergias) son consultables por Administrador, Personal administrativo y Prefecto; el adeudo de pago y las materias pendientes son consultables únicamente por Administrador y Personal administrativo. | Consulta o edición del expediente | Los campos fuera del alcance del rol no se incluyen en la respuesta de la API ni en la interfaz. | Estos datos están sujetos a control de cambios respecto a la Matriz de Alcance del acta (ver P3-A-01). | RF-EST-06, RN-TRX-01, RN-TRX-02, LFPDPPP |
...
| RN-IMP-08 | RES | El grado, grupo, ciclo escolar y turno de una importación se seleccionan una sola vez para todo el lote (no son columnas del archivo) y se aplican a todas las filas válidas de ese lote. | Inicio de una importación | Toda fila insertada del lote queda asociada al grupo, ciclo y turno seleccionados; un lote no puede mezclar estudiantes de distinto grupo. | Si la institución entrega un archivo con estudiantes de varios grupos, debe dividirse en varios lotes, uno por grupo. | RF-IMP-01, RN-IMP-06 |
...
| RN-CRE-01 | RES | Solo se exporta el QR de estudiantes y docentes en estatus `ACTIVO` con QR vigente. Para exportar una **credencial completa** se exige además la fotografía cargada; para un **sticker** de QR no se exige. | Solicitud de exportación de credenciales o stickers | Se excluyen de la exportación quienes no cumplan la condición y se listan aparte como pendientes, con el motivo. | — | RF-CRE-01, RF-CRE-02, RF-EST-01, RF-QR-04, RN-AST-27 |
...
| Alta, edición y desactivación de docentes · programación · historial docente | ✓ | — | — | ✓ | — |
| Consultar fichas de docentes (búsqueda y filtrado) | ✓ | — | — | ✓ | — |
| Reactivar docente (RN-AST-28) · vincular docente con cuenta de usuario | ✓ | — | — | — | — |
...
| RN-AST-23 | DEF | Un docente es una persona registrada en SIGE mediante una ficha propia (expediente docente), independiente de las cuentas de usuario; no requiere cuenta para que se registre su asistencia. Solo los docentes `ACTIVO` con programación vigente son evaluados y pueden registrar asistencia. | Alta, edición, desactivación o reactivación de un docente | Se guarda la ficha con su estatus e historial; un docente `INACTIVO` conserva su historial (archivado lógico) y deja de evaluarse. | — | RF-AST-11, RN-AST-26 a RN-AST-30, RN-TRX-03 |
...
| RN-AST-26 | RES | El identificador interno del docente (clave institucional) es obligatorio, único e inmutable una vez asignado; la unicidad incluye a los docentes `INACTIVO`. | Alta o edición de un docente | Se rechaza el alta con un identificador ya existente y cualquier intento de modificar el de una ficha existente. | Corrección excepcional por Personal administrativo o Administrador, con motivo y registro en bitácora. | RF-AST-11, RN-AUT-05 |
| RN-AST-27 | DEF | La ficha del docente contiene datos **mínimos** (nombre completo, identificador interno y estatus) y datos **opcionales** (fotografía, correo electrónico y teléfono). No se recolectan otros datos personales del docente salvo que un módulo del alcance los requiera. | Alta o edición de un docente | Se rechaza el guardado sin los datos mínimos; los datos opcionales no bloquean el guardado ni generan marca de incompleto. | La fotografía es obligatoria únicamente para exportar una credencial completa (RN-CRE-01). | RF-AST-11, RN-TRX-01 |
| RN-AST-28 | RES | Un docente tiene exactamente un estatus: `ACTIVO` o `INACTIVO`. Las únicas transiciones válidas son `ACTIVO → INACTIVO` (Personal administrativo o Administrador) e `INACTIVO → ACTIVO` (solo Administrador); ambas exigen motivo. Un docente nunca se elimina. | Cambio de estatus de un docente | Se registra un evento de auditoría con usuario, fecha/hora, estatus anterior, estatus nuevo y motivo; la reactivación queda como evento distinto del alta original. Se rechaza el cambio hecho por un rol no autorizado. | — | RF-AST-11, RN-AUT-05, RN-EST-06, RN-TRX-03 |
| RN-AST-29 | HAB | Al desactivar a un docente: (a) deja de aparecer en las operaciones activas (verificación de ausencias, registro de `ENTRADA` y `SALIDA`); (b) su programación se conserva suspendida; (c) su QR vigente **no se revoca**: los escaneos se rechazan por RN-AST-23 y el QR se conserva si se reactiva; (d) su historial de asistencia sigue consultable. | Cambio a `INACTIVO` | Se aplican los efectos (a) a (d). Si el docente tiene una cuenta de usuario vinculada y activa, SIGE advierte al Administrador para que decida sobre ella; la cuenta no se desactiva automáticamente. | — | RF-AST-11, RF-QR-04, RN-AST-23, RN-QR-06 |
| RN-AST-30 | DEF | Un docente puede vincularse a una sola cuenta de usuario con rol Docente (relación 1 a 1), y una cuenta con rol Docente solo puede vincularse a un docente. Una cuenta con rol Docente debe estar vinculada a un docente `ACTIVO` para registrar reportes (RN-REP-09). | Vinculación de una cuenta con un docente | Se rechaza vincular una cuenta a más de un docente o un docente a más de una cuenta; sin vínculo (o con el docente `INACTIVO`) la cuenta no puede registrar reportes. | La asistencia del docente no depende del vínculo (RN-AST-23). | RF-AST-11, RF-AUT-03, RN-REP-09, RN-AUT-01 |
...
| RN-REP-09 | RES | Cada grupo tiene uno o más prefectos asignados y cada docente tiene los grupos que atiende (catálogos de RF-ADM-01). Un docente solo puede registrar reportes de estudiantes `ACTIVO` de sus grupos asignados; el reporte se envía a los prefectos asignados al grupo del estudiante. | Registro de reporte | Se rechaza el registro fuera de los grupos asignados. Si el grupo no tiene prefecto asignado, el reporte queda en la bandeja del Personal administrativo y del Administrador y se genera alerta. El docente que registra el reporte es el docente `ACTIVO` vinculado a la cuenta del usuario (RN-AST-30). | — | RF-REP-01, RF-REP-03, RF-ADM-01, RN-TRX-02,RN-AST-30 |
...
| RN-AST-23 / RN-AST-28 / RN-AST-29 | Secuencia | Desactivar a un docente lo excluye de la evaluación y de las operaciones activas, conserva su historial, su programación y su QR; solo el Administrador puede reactivarlo. |
| RN-AST-30 / RN-REP-09 | Dependencia | Sin vínculo con un docente `ACTIVO`, una cuenta con rol Docente no puede registrar reportes. |
| RN-EST-11 / RN-COM-01 | Precondición | Un tutor con contacto válido (correo o teléfono) no garantiza que pueda recibir comunicaciones automatizadas; eso exige específicamente que tenga correo (RN-COM-01). | | RN-EST-12 / RN-TRX-02 | Refuerzo | El acceso a salud y adeudos sigue el mismo principio de "ningún rol por defecto" que ya rige el resto de los datos de menores. |
